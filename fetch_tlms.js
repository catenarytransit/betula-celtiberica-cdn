...
.then((response) => response.json())
 .then((data) => {
        let agency_ids = data.filter((a) => a.agency_name == "DVB-Straßenbahn" || a.agency_name == "DVB-Bus").map((x) => x.agency_id)

         fetch("https://birch.catenarymaps.org/getroutesofchateau/deutschland")
    .then((response) => response.json())
    .then((data) => {
        let results = data.filter((r) => agency_ids.includes(r.agency_id)).map((x) => {return {route_id: x.route_id, short_name: x.short_name}}).sort((a, b) => a.route_id - b.route_id);
